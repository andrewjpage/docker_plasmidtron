...
# assemble_with_spades.sh /home/ubuntu/docker_plasmidtron /data /data/data
...
for FORWARD_FILE in $(find ${HOST_BASE} -type f -name "*_1.fastq.gz");
  do
    ...
    if [ ! -f ${HOST_BASE}/plasmidspades/${BASE_NAME}/contigs.fasta ]; then
       { time docker run -it --rm -v ${HOST_BASE}:/data ${DOCKER_HASH} spades.py --plasmid -t 1 -1 ${FORWARD_FILE} -2 ${REVERSE_FILE} -o ${OUTPUT_DIRECTORY}/${BASE_NAME} --phred-offset 33 ; }  2> ${HOST_BASE}/plasmidspades/timings_${BASE_NAME}
    fi
